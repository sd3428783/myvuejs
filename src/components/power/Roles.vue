<template>
  <div>
    ...
    <!-- 修改角色的对话框 -->
    <el-dialog title="修改角色" :visible.sync="editRoleDialogVisible" width="50%" @close="editDialogClosed">
      <el-form :model="roleEditForm" :rules="editFormRules" ref="editFormRef" label-width="70px">
  ...
  width="50%" @close="addRoleFormClosed">
  <!-- 添加角色的表单 -->
...
export default {
  ...
  methods: {
    ...
    async showEditDialog(id) {
      ...
      if(res.meta.status !== 200) {
        return this.$message.error('角色数据加载失败')
      }
    ...
    editRoleInfo() {
      this.$refs.editFormRef.validate(async valid => {
        const {data:res} = await this.$http.put('roles/' + this.roleEditForm.roleId, {
